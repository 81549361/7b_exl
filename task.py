...
all_time = 0;
stop_task = False
model_directory = "/workspace/toppydpo_checkpoint300-8.0"
draft_directory = "/workspace/toppydpo_checkpoint300-4"
# Load LoRA

lora_directory = "/workspace/hiwaifu7b/checkpoint-300/"
lora = None

def load_model():
    global generator, default_settings,tokenizer,lora
    if not generator:
        model_config = ExLlamaV2Config()
        model_config.model_dir = model_directory
        model_config.prepare()
        #model_config.max_seq_len = 4096
        print(model_config.max_seq_len)
...
def remove_last_unpaired_quote(text):
...
def generate(params: Item):
    ...
    try:
        ...
        conv = Conversation(name="mpt-7b-chat",system_template="""<|im_start|>system
{system_message}""",sep="<|im_end|>",
        stop_token_ids=[50278, 0], system_message=params.system, messages=params.messages, roles = (params.role2,params.role1))
        #conv = get_conv_template("alpaca")
        conv.append_message(params.role2, params.text)
        conv.append_message(params.role1+"\n"+params.bot_name + ":", None)
        #print(conv.get_prompt())
        prompt = conv.get_prompt(tokenizer,4096)
        print(prompt)
    except Exception as e:
        traceback.print_exc()
    try:
        ...
        gen_settings.temperature = 2
        #gen_settings.top_k = params.top_k
        gen_settings.top_k = 0
        #gen_settings.top_p = params.top_p
        gen_settings.top_p = 0
        gen_settings.min_p = 0.05
        #gen_settings.tfs = 1
        gen_settings.mirostat = True
        gen_settings.token_repetition_penalty = 1.03
        #gen_settings.token_repetition_range = 2048
        
        gen_settings.disallow_tokens(tokenizer, [tokenizer.eos_token_id])
        
        input_ids = tokenizer.encode(prompt)
        prompt_tokens = input_ids.shape[-1]
        generator.set_stop_conditions(["You:","User:","</","<|", "\n#", "###", "\n``", "\n\n\n", "\n[", "\n(", f"{params.bot_name}:",f"{params.user_name}:"])
        ...
        while True:
            ...
            generated_tokens += 1
            if (generated_tokens>=170 and text.count('"') % 2 == 1 and chunk.find('"') != -1) or (generated_tokens>=200 and text.count('*') % 2 == 1 and chunk.find('*') != -1):
                text = text + chunk
                text = remove_last_unpaired_quote(text)
