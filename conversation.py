...
class Conversation:
    ...
    def get_prompt(self,tokenizer,context_len=3300) -> str:
            ...
            while True:
                for role, message in self.messages:
                    if message:
                        ...
                        ret += role + ":"
                token_count = len(tokenizer.encode(ret)[0])
                if token_count > context_len:
                    del self.messages[0]
                    ret = system_prompt + self.sep
                else:
                    break
            #print(f"1上下文：{token_count}")
            return ret
